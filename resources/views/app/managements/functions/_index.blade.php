...
@section('content')
<h3 class="text-gray-700 text-3xl font-medium">Functions</h3>
<div class="container bg-white p-10 my-10" x-data="functionsCrud()">
    ...
    <x-modal :value="1">
        ...
        <div class="border-b-2 border-black mb-4">
            <h2 class="text-3xl font-medium" :id="$id('modal-title')">Form Data Functions</h2>
        </div>
        <!-- Content -->
        <form action="#" @submit.prevent="confirmSave">
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full px-3 my-3">
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="function_name">
                        Function Name
                    </label>
                    <input name="function_name" x-model="form.function_name"
                    class="w-full h-10 px-3 text-base text-gray-700 placeholder-gray-300 border rounded-lg focus:shadow-outline" 
                    type="text" 
                    placeholder="Function Name">
                    <small x-text="errMsg.function_name" class="ml-3 text-xs text-red-500"></small>
                </div>
                <div class="w-full px-3 my-3">
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="function_desc">
                        Description
                    </label>
                    <textarea name="function_desc" x-model="form.function_desc"
                    class="w-full h-24 px-3 py-2 text-base text-gray-700 placeholder-gray-300 border rounded-lg focus:shadow-outline" 
                    type="text" 
                    placeholder="Description">
                    </textarea>
                    <small x-text="errMsg.function_desc" class="ml-3 text-xs text-red-500"></small>
                </div>
            </div>
            <!-- Buttons -->
        ...
        <x-slot:tbody>
            <template x-for="(row, index) in datatable.data" :key="index">
                <tr x-show="!datatable.loading" class="border-b dark:border-gray-700">
                    ...
                    <td class="px-4 py-3" x-text="row.function_name"></td>
                    <td class="px-4 py-3" x-text="row.function_desc"></td>
...
@section('_inJs')
@include('app.managements.functions._inJs')
@endsection
<!-- /Your Custom Javascript -->