...
@section('content')
<h3 class="text-gray-700 text-3xl font-medium">Modules</h3>
<div class="container bg-white p-10 my-10" x-data="modulesCrud()">
    ...
    <x-modal :value="1">
        ...
        <div class="border-b-2 border-black mb-4">
            <h2 class="text-3xl font-medium" :id="$id('modal-title')">Form Data Modules</h2>
        </div>
        <!-- Content -->
        <form action="#" @submit.prevent="confirmSave">
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full px-3 my-3">
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="module_name">
                        Module Name
                    </label>
                    <input name="module_name" x-model="form.module_name"
                    class="w-full h-10 px-3 text-base text-gray-700 placeholder-gray-300 border rounded-lg focus:shadow-outline" 
                    type="text" 
                    placeholder="Module Name">
                    <small x-text="errMsg.module_name" class="ml-3 text-xs text-red-500"></small>
                </div>
                <div class="w-full px-3 my-3">
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="module_desc">
                        Description
                    </label>
                    <textarea name="module_desc" x-model="form.module_desc"
                    class="w-full h-24 px-3 py-2 text-base text-gray-700 placeholder-gray-300 border rounded-lg focus:shadow-outline" 
                    type="text" 
                    placeholder="Description">
                    </textarea>
                    <small x-text="errMsg.module_desc" class="ml-3 text-xs text-red-500"></small>
                </div>
            </div>
            <!-- Buttons -->
        ...
        <x-slot:tbody>
            <template x-for="(row, index) in datatable.data" :key="index">
                <tr x-show="!datatable.loading" class="border-b dark:border-gray-700">
                    ...
                    <td class="px-4 py-3" x-text="row.module_name"></td>
                    <td class="px-4 py-3" x-text="row.module_desc"></td>
                    ...
                    <td class="px-4 py-3 flex items-center">
                        ...
                        <a class="inline-flex items-center justify-center w-8 h-8 mr-2 text-indigo-100 transition-colors duration-150 bg-blue-500 rounded-full focus:shadow-outline hover:bg-blue-600" 
                            :href="`/managements/actions/${row.id}`">
...
@section('_inJs')
@include('app.managements.modules._inJs')
@endsection
<!-- /Your Custom Javascript -->