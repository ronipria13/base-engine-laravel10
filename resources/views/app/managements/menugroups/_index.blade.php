...
@section('content')
<h3 class="text-gray-700 text-3xl font-medium">Menu Group</h3>
<div class="container bg-white p-10 my-10" x-data="menugroupsCrud()">
    ...
    <x-modal :value="1">
        ...
        <div class="border-b-2 border-black mb-4">
            <h2 class="text-3xl font-medium" :id="$id('modal-title')">Form Data Menu Group</h2>
        </div>
        <!-- Content -->
        <form action="#" @submit.prevent="confirmSave">
            <div class="flex flex-wrap -mx-3 mb-6">
                <div class="w-full px-3 my-3">
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="menugroup_label">
                        Label
                    </label>
                    <input name="menugroup_label" x-model="form.menugroup_label"
                    class="w-full h-10 px-3 text-base text-gray-700 placeholder-gray-300 border rounded-lg focus:shadow-outline" 
                    type="text" 
                    placeholder="Label">
                    <small x-text="errMsg.menugroup_label" class="ml-3 text-xs text-red-500"></small>
                </div>
                <div class="w-full px-3 my-3">
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="menugroup_desc">
                        Description
                    </label>
                    <textarea name="menugroup_desc" x-model="form.menugroup_desc"
                    class="w-full h-24 px-3 py-2 text-base text-gray-700 placeholder-gray-300 border rounded-lg focus:shadow-outline" 
                    type="text" 
                    placeholder="Description">
                    </textarea>
                    <small x-text="errMsg.menugroup_desc" class="ml-3 text-xs text-red-500"></small>
                </div>
                <div class="w-full px-3 my-3">
                    <label class="block tracking-wide text-gray-700 text-sm font-bold mb-2" for="menugroup_order">
                        Urutan
                    </label>
                    <input name="menugroup_order" x-model="form.menugroup_order"
                    class="w-full h-10 px-3 py-2 text-base text-gray-700 placeholder-gray-300 border rounded-lg focus:shadow-outline" 
                    type="number" 
                    placeholder="Urutan">
                    <small x-text="errMsg.menugroup_order" class="ml-3 text-xs text-red-500"></small>
                </div>
            </div>
            <!-- Buttons -->
        ...
        <x-slot:tbody>
            <template x-for="(row, index) in datatable.data" :key="index">
                <tr x-show="!datatable.loading" class="border-b dark:border-gray-700">
                    <td class="px-4 py-3 text-right" x-text="row.menugroup_order"></td>
                    <td class="px-4 py-3" x-text="row.menugroup_label"></td>
                    <td class="px-4 py-3" x-text="row.menugroup_desc"></td>
...
@section('_inJs')
@include('app.managements.menugroups._inJs')
@endsection
<!-- /Your Custom Javascript -->